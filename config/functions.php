...
class functions {
	...
	public function add_officer($data)
	{
			...
			if ( $this->check_availability("SELECT * FROM tbluser WHERE username = '$username'") ) {
				$this->notif("Gagal! petugas sudah ada","warning");
				$this->redirect($this->baseurl . "officer_add.php");
			} else {
				$insert = $this->exe("INSERT INTO tbluser VALUES ('','$nama','$username','$password','$privilege')");
				if ( $insert > 0 ) {
					$this->notif("Sukses menambah petugas","success");
	...
	public function delete_officer($id_user)
	{
			$delete = $this->exe("DELETE FROM tbluser WHERE id_user = '$id_user'");
			if ( $delete > 0 ) {
				$this->notif("Sukses menghapus petugas","success");
				$this->redirect($this->baseurl . "officer.php");
			} else {
				$this->notif("Gagal! Kesalahan pada query","danger");
				$this->redirect($this->baseurl . "officer.php");
